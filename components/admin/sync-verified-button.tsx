        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Синхронизация verified/confidence
            </DialogTitle>
          ...
          {result?.success && result.data ? (
            <div className="space-y-4">
              <div className="rounded-lg border p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="rounded bg-muted p-3">
                    ...
                    <div className="text-xs text-muted-foreground text-center">verified=true → confidence=1.0</div>
                  </div>
                  <div className="rounded bg-muted p-3">
                    <div className="text-2xl font-bold text-center">{result.data.updatedConfidence}</div>
                    <div className="text-xs text-muted-foreground text-center">confidence=1.0 → verified=true</div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-green-200 bg-green-50 p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-green-700">Verified=true:</span>
                    <span className="ml-2 font-bold text-green-800">{result.data.totalVerified.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-green-700">Confidence=1.0:</span>
              ...
              {result.data.updatedVerified === 0 && result.data.updatedConfidence === 0 && (
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-center">
                  ...
                  <p className="text-sm text-blue-600">Поля verified и confidence согласованы</p>
                </div>
              )}
            </div>
          ) : (
