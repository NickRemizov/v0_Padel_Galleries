...
export function GalleryList({ galleries: initialGalleries, photographers, locations, organizers, onDelete, onUpdate }: GalleryListProps) {
  ...
  async function handleDeleteConfirm() {
    if (!deleteConfirm.gallery) return
    const id = deleteConfirm.gallery.id
  ...
  return (
    <>
              ...
              <CardContent className="flex items-center gap-4 p-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 flex gap-4 text-xs text-muted-foreground">
                    ...
                    <span>Обложка: {gallery.featured_count ?? 0} featured / {gallery.vertical_count ?? 0} вертикальных</span>
                  </div>
                  <div className="mt-1 flex gap-4 text-xs text-muted-foreground">
      ...
      <DeleteConfirmDialog
        ...
        onOpenChange={(open) => setDeleteConfirm({ open, gallery: open ? deleteConfirm.gallery : null })}
