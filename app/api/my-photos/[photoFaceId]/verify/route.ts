...
export async function POST(
  ...
  try {
    ...
    const result = await apiFetch(
      `/api/user/photo-faces/${photoFaceId}/verify?person_id=${user.person_id}&user_id=${user.id}`,
      { method: "POST" }
    )
    ...
    if (!result.success) {
      ...
      return NextResponse.json(
        { error: result.error || "Failed to verify" },
        { status: 400 }
      )
    }

    console.log(`[my-photos] User ${user.id} verified photo_face ${photoFaceId} via FastAPI`)
