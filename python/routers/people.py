...
async def unlink_person_from_photo(identifier: str, photo_id: str = Query(...)):
    ...
    try:
        ...
        supabase_db_instance.client.table("photo_faces")\
            .update({"person_id": None, "verified": False, "verified_at": None, "verified_by": None, "recognition_confidence": None})\
            .eq("photo_id", photo_id)\
            .eq("person_id", person_id)\
            .execute()
...
async def clear_person_outliers(identifier: str, outlier_threshold: float = Query(0.5)):
    ...
    try:
        ...
        if len(faces) < 2:
            return ApiResponse.ok({"cleared_count": 0, "message": "Need at least 2 embeddings to find outliers"})
        ...
        if len(embeddings_data) < 2:
            return ApiResponse.ok({"cleared_count": 0, "message": "Not enough valid embeddings"})
        
        non_excluded = [(e, f) for e, f in embeddings_data if not f.get("excluded_from_index")]
        
        if len(non_excluded) < 2:
            return ApiResponse.ok({"cleared_count": 0, "message": "Not enough non-excluded embeddings for centroid"})
        ...
        if not outlier_face_ids:
            return ApiResponse.ok({"cleared_count": 0, "message": "No outliers found"})
        ...
        return ApiResponse.ok({"cleared_count": updated, "index_rebuilt": index_rebuilt})
...
async def get_person_photos_with_details(identifier: str):
    """Get photos with detailed info including excluded_from_index."""
    try:
        person_id = _get_person_id(identifier)
        ...
        photo_faces_result = supabase_db_instance.client.table("photo_faces")\
            .select("id, photo_id, recognition_confidence, verified, insightface_bbox, person_id, excluded_from_index, "
        ...
        photo_faces = [pf for pf in all_photo_faces if pf.get("verified") == True or (pf.get("recognition_confidence") or 0) >= confidence_threshold]
        ...
        photos_with_other_faces = [{**photo, "otherFaces": other_faces_by_photo.get(photo["id"], [])} for photo in photos]
...
async def get_embedding_consistency(identifier: str, outlier_threshold: float = Query(0.5)):
